...
def test_authenticate_user_failure(db_session, mock_user):
...
def test_get_user_by_username(db_session, mock_user):
...
@pytest.mark.asyncio
async def test_get_current_user_success(db_session, mock_user, auth_token):
...
async def test_get_current_user_invalid_token(db_session):
    ...
    invalid_token = create_access_token("wrongUser")
...
@pytest.mark.asyncio
async def test_get_current_user_invalid_deocde_token(db_session):
    """Testa a falha ao tentar obter um usuário com um token inválido."""
